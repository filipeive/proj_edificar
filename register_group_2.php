...
// Update package responsible
$package = CommitmentPackage::find($packageId);
if ($package) {
    $package->update(['responsible_id' => $responsibleId]);
    echo "Package $packageId updated with responsible $responsibleId\n";
}
    ...
    try {
        ...
        if (!$user) {
            ...
            $user = User::create([
                'name' => $data['name'],
                'email' => $email,
                'password' => bcrypt('edificar123'),
                'phone' => $data['phone'],
                'role' => 'membro',
                'is_active' => true,
            ]);
            echo "Created User: {$user->name} ({$user->id})\n";
        } else {
            echo "Existing User: {$user->name} ({$user->id})\n";
        }

        // Check if commitment already exists
        $exists = UserCommitment::where('user_id', $user->id)
        ...
        if (!$exists) {
            ...
            echo "Added commitment for {$user->name}\n";
        } else {
            echo "Commitment already exists for {$user->name}\n";
        }
    } catch (\Exception $e) {
        echo "Error registering {$data['name']}: " . $e->getMessage() . "\n";
    }
}
