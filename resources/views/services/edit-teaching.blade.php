...
@section('content')
    <div class="space-y-8" x-data="{ 
        ...
        <div
            ...
            <div>
                <div class="flex items-center gap-2 text-xs font-bold text-blue-600 uppercase tracking-widest mb-2">
                    ...
                    <span>Editar Culto de Ensino</span>
                </div>
                <h1 class="text-3xl font-black text-gray-900 tracking-tight">Editar Culto de Ensino</h1>
            </div>
        ...
        <form action="{{ route('services.update', $service) }}" method="POST" id="serviceForm" class="space-y-8 pb-12">
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Data do
                            Culto</label>
                        <input type="date" name="date" value="{{ old('date', $service->date->format('Y-m-d')) }}" required
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="space-y-4">
                            <select name="preacher_id" @change="guestPreacher = $event.target.value === 'other'"
                                ...
                                @endforeach
                                <option value="other" @selected(old('preacher_id', $service->preacher_id) === 'other' || ($service->preacher_id === null && $service->preacher_name))>Outro (Convidado)</option>
                            </select>
                    ...
                    <div class="md:col-span-2 space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Tema da
                            Mensagem</label>
                        <input type="text" name="theme" value="{{ old('theme', $service->theme) }}"
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Visitantes Gerais (Adultos)</label>
                        <input type="number" name="adults_visitors" value="{{ old('adults_visitors', $service->adults_visitors) }}" min="0"
                            class="w-full px-5 py-4 bg-gray-50 border-transparent focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 rounded-2xl transition-all font-bold text-gray-700">
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Visitantes Gerais (Crianças)</label>
                        <input type="number" name="children_visitors" value="{{ old('children_visitors', $service->children_visitors) }}" min="0"
                            class="w-full px-5 py-4 bg-gray-50 border-transparent focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 rounded-2xl transition-all font-bold text-gray-700">
                    </div>
                </div>
            </div>
            ...
            <!-- Sunday metrics (Keep hidden and set to 0 for teaching services) -->
            <!-- Sunday metrics (Keep hidden and set to 0 for teaching services) -->
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-8 space-y-4">
                        @foreach($offeringTypes as $index => $type)
                            @php
                                $offering = $service->offerings->where('offering_type_id', $type->id)->first();
                            @endphp
                            <div
                                ...
                                <div class="relative w-40">
                                    ...
                                    <input type="number" step="0.01" name="offerings[{{ $index }}][amount]"
                                        ...
                                        value="{{ old("offerings.{$index}.amount", $offering ? $offering->amount : 0) }}">
                                    <span
                        ...
                        <div class="space-y-4 pt-4 mt-8 border-t border-gray-50">
                            <div class="flex items-center gap-4 group">
                                <div class="flex-1">
                                    <p class="text-[10px] font-black text-orange-600 uppercase tracking-widest">Ofert. Especial</p>
                                ...
                                <div class="relative w-40">
                                    <input type="number" step="0.01" name="special_offerings_total" id="special_offer_input"
                                        ...
                                        value="{{ old('special_offerings_total', $service->special_offerings_total) }}">
                                    <span
                ...
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                    ...
                    <div class="p-8 flex-1 space-y-4 max-h-[500px] overflow-y-auto" id="contributionsContainer">
                        @foreach(old('individual_contributions', $service->tithes->map(fn($t) => ['type' => 'tithe', 'member_name' => $t->member_name, 'amount' => $t->amount, 'description' => $t->description])
                            ->concat($service->individualOfferings->map(fn($o) => ['type' => 'offering', 'member_name' => $o->member_name, 'amount' => $o->amount, 'description' => $o->description]))) as $index => $contribution)
                            <div class="contribution-row space-y-2 p-4 bg-gray-50 rounded-2xl group relative border border-transparent hover:border-blue-100 transition-all">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="relative w-28">
                                        <input type="number" step="0.01" name="individual_contributions[{{ $index }}][amount]" 
                                    ...
                                    <button type="button" class="remove-contribution text-gray-300 hover:text-red-500 transition-colors bg-white hover:bg-red-50 p-2 rounded-lg">
                                        <i class="bi bi-trash"></i>
                                    </button>
                                </div>
                                <div class="relative">
                                    <i class="bi bi-chat-left-text absolute left-3 top-1/2 -translate-y-1/2 text-gray-300 text-xs"></i>
                                    <input type="text" name="individual_contributions[{{ $index }}][description]" 
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                <div class="lg:col-span-2 bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8">
                    <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1 mb-4 block">Comentários e Observações</label>
                    <textarea name="observations" rows="6" placeholder="Fale sobre o mover, testemunhos ou ocorrências do culto..."
                        class="w-full px-6 py-4 bg-gray-50 border-transparent focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 rounded-[2rem] transition-all font-medium text-gray-700 placeholder-gray-300 resize-none">{{ old('observations', $service->observations) }}</textarea>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let contributionIndex = {{ count(old('individual_contributions', $service->tithes->map(fn($t) => ['type' => 'tithe', 'member_name' => $t->member_name, 'amount' => $t->amount, 'description' => $t->description])
                ->concat($service->individualOfferings->map(fn($o) => ['type' => 'offering', 'member_name' => $o->member_name, 'amount' => $o->amount, 'description' => $o->description])))) }};
            ...
            // Re-attach listeners to existing rows
            ...
            function updateFinancials() {
                ...
                if(document.getElementById('total_offerings_display')) document.getElementById('total_offerings_display').innerText = formatMT(offeringsTotal);
                if(document.getElementById('total_tithes_display')) document.getElementById('total_tithes_display').innerText = formatMT(tithesTotal);
                if(document.getElementById('total_ind_offerings_display')) document.getElementById('total_ind_offerings_display').innerText = formatMT(indOfferingsTotal);
                if(document.getElementById('total_contributions_display')) document.getElementById('total_contributions_display').innerText = formatMT(tithesTotal + indOfferingsTotal);
