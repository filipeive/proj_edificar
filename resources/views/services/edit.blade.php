...
@section('page-subtitle', 'Atualize os dados do culto e as ofertas coletadas')
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="date" name="date" value="{{ old('date', $service->date->format('Y-m-d')) }}"
                                    required
                            ...
                            <div>
                                ...
                                <select name="service_type" required
                                    class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                                    <option value="1st" {{ old('service_type', $service->service_type) == '1st' ? 'selected' : '' }}>1º Culto</option>
                                    <option value="2nd" {{ old('service_type', $service->service_type) == '2nd' ? 'selected' : '' }}>2º Culto</option>
                                    <option value="3rd" {{ old('service_type', $service->service_type) == '3rd' ? 'selected' : '' }}>3º Culto</option>
                                    <option value="4th" {{ old('service_type', $service->service_type) == '4th' ? 'selected' : '' }}>4º Culto</option>
                                    <option value="special" {{ old('service_type', $service->service_type) == 'special' ? 'selected' : '' }}>Especial</option>
                                </select>
                            ...
                            <div>
                                ...
                                <select name="preacher_id"
                                    ...
                                    @foreach($preachers as $preacher)
                                        <option value="{{ $preacher->id }}" {{ old('preacher_id', $service->preacher_id) == $preacher->id ? 'selected' : '' }}>
                                            {{ $preacher->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="message" rows="4"
                                class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500">{{ old('message', $service->message) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="adults_count"
                                    value="{{ old('adults_count', $service->adults_count) }}" min="0" required
                                    class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <div>
                                ...
                                <input type="number" name="children_count"
                                    value="{{ old('children_count', $service->children_count) }}" min="0" required
                                    class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="space-y-6">
                        ...
                        <div class="space-y-4">
                            @foreach($offeringTypes as $index => $type)
                                @php
                                    ...
                                    $amount = $offering ? $offering->amount : 0;
                                @endphp
                                <div class="p-4 bg-gray-50 rounded-lg border border-gray-100">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" step="0.01" name="offerings[{{ $index }}][amount]"
                                            value="{{ old("offerings.{$index}.amount", $amount) }}" min="0"
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <textarea name="observations" rows="3" placeholder="Notas adicionais..."
                            class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500">{{ old('observations', $service->observations) }}</textarea>
                    </div>
                    ...
                    <button type="submit"
                        ...
                        <i class="bi bi-check-lg mr-2"></i> ATUALIZAR CULTO
                    </button>
                </div>
            </div>
        </form>
