    ...
    <div class="space-y-8" x-data="{ 
        ...
        <div
            ...
            <div>
                ...
                <h1 class="text-3xl font-black text-gray-900 tracking-tight">Editar Celebração</h1>
            </div>
        ...
        <form action="{{ route('services.update', $service) }}" method="POST" id="serviceForm" class="space-y-8 pb-12">
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="service_type" required x-model="serviceType"
                            ...
                            <option value="4th" @selected($service->service_type == '4th')>4º Culto</option>
                            <option value="teaching" @selected($service->service_type == 'teaching')>Culto de Ensino (4ª
                                Feira)</option>
                            <option value="special" @selected($service->service_type == 'special')>Especial</option>
                        </select>
                    </div>
            ...
            <div x-show="serviceType === 'teaching'" x-transition
                ...
                <div class="p-8 border-b border-gray-50 bg-blue-50/30 flex items-center justify-between">
                    <h2 class="text-lg font-black text-gray-900 flex items-center gap-2">
                        ...
                        Participação por Zona (Culto de Ensino)
                    </h2>
                </div>
                                ...
                                <tr class="group">
                                    ...
                                    <td class="p-2 bg-gray-50/50 group-hover:bg-white transition-colors">
                                        <input type="number" name="zone_participations[{{ $index }}][adults_members]"
                                            value="{{ old("zone_participations.{$index}.adults_members", $part ? $part->adults_members : 0) }}"
                                            min="0"
                                    ...
                                    <td class="p-2 bg-gray-50/50 group-hover:bg-white transition-colors">
                                        <input type="number" name="zone_participations[{{ $index }}][adults_visitors]"
                                            value="{{ old("zone_participations.{$index}.adults_visitors", $part ? $part->adults_visitors : 0) }}"
                                            min="0"
                                            class="zone-participation-input w-full bg-transparent border-transparent focus:ring-0 text-center font-black text-gray-700">
                                    </td>
                    ...
                    <table class="w-full border-separate border-spacing-2">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="group">
                                ...
                                <td class="p-2 bg-gray-50/50 group-hover:bg-white transition-colors">
                                    <input type="number" name="adults_members"
                                        value="{{ old('adults_members', $service->adults_members) }}" min="0"
                                        class="participation-input w-full bg-transparent border-transparent focus:ring-0 text-center font-black text-gray-700">
                                </td>
                                <td class="p-2 bg-gray-50/50 group-hover:bg-white transition-colors">
                                    <input type="number" name="adults_visitors"
                                        value="{{ old('adults_visitors', $service->adults_visitors) }}" min="0"
                                        class="participation-input w-full bg-transparent border-transparent focus:ring-0 text-center font-black text-gray-700">
                                </td>
                                <td class="p-2 bg-gray-50/50 group-hover:bg-white transition-colors">
                                    <input type="number" name="adults_salvations"
                                        value="{{ old('adults_salvations', $service->adults_salvations) }}" min="0"
                            ...
                            <tr class="group">
                                ...
                                <td class="p-2 bg-gray-50/50 group-hover:bg-white transition-colors">
                                    <input type="number" name="children_members"
                                        value="{{ old('children_members', $service->children_members) }}" min="0"
                                        class="participation-input w-full bg-transparent border-transparent focus:ring-0 text-center font-black text-gray-700">
                                </td>
                                <td class="p-2 bg-gray-50/50 group-hover:bg-white transition-colors">
                                    <input type="number" name="children_visitors"
                                        value="{{ old('children_visitors', $service->children_visitors) }}" min="0"
                                        class="participation-input w-full bg-transparent border-transparent focus:ring-0 text-center font-black text-gray-700">
                                </td>
                                <td class="p-2 bg-gray-50/50 group-hover:bg-white transition-colors">
                                    <input type="number" name="children_salvations"
                                        value="{{ old('children_salvations', $service->children_salvations) }}" min="0"
                                        class="participation-input w-full bg-transparent border-transparent focus:ring-0 text-center font-black text-orange-600">
                                </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateZoneParticipation() {
                const teachingMembersEl = document.getElementById('total_teaching_members');
                if (!teachingMembersEl) return;

                let totalMembers = 0;
                let totalVisitors = 0;

                document.querySelectorAll('.group').forEach(row => {
                    const membersInput = row.querySelector('input[name*="adults_members"].zone-participation-input');
                    const visitorsInput = row.querySelector('input[name*="adults_visitors"].zone-participation-input');
                    const rowTotalDisplay = row.querySelector('.zone-row-total');

                    if (membersInput && visitorsInput && rowTotalDisplay) {
                        const members = parseInt(membersInput.value) || 0;
                        const visitors = parseInt(visitorsInput.value) || 0;
                        const rowTotal = members + visitors;
                ...
                teachingMembersEl.innerText = totalMembers;
                document.getElementById('total_teaching_visitors').innerText = totalVisitors;
                document.getElementById('total_teaching_grand').innerText = totalMembers + totalVisitors;
            }

            document.querySelectorAll('.zone-participation-input').forEach(input => {
