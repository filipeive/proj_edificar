...
@section('title', 'Relatório da Supervisão - Portal Life Church')
@section('page-title', 'Relatório da Supervisão')
@section('page-subtitle', $supervision ? $supervision->name : 'Selecione uma supervisão para começar')
...
<div class="bg-white rounded-lg shadow p-6 mb-6">
    ...
    <form method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        ...
        <div class="lg:col-span-1">
            <label for="supervision_id" class="block text-sm font-medium text-gray-700">Selecionar Supervisão</label>
            <select name="supervision_id" id="supervision_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" onchange="this.form.submit()">
                <option value="">-- Escolha uma supervisão --</option>
                @foreach($allSupervisions as $s)
                <option value="{{ $s->id }}" @if($supervision && $supervision->id == $s->id) selected @endif>{{ $s->name }}</option>
                @endforeach
            </select>
        </div>
        @endif
...
<div class="bg-white rounded-lg shadow overflow-hidden">
    ...
    <table class="min-w-full">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            @empty
            <tr>
                <td colspan="5" class="px-6 py-12 text-center text-gray-500">Nenhuma contribuição encontrada para este período.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
...
@else
    ...
    <div class="text-center py-12 bg-white rounded-lg shadow">
        ...
        <h3 class="mt-4 text-lg font-medium text-gray-900">Nenhuma supervisão selecionada</h3>
        <p class="mt-1 text-sm text-gray-500">Por favor, escolha uma supervisão no filtro acima para visualizar o relatório.</p>
    </div>
    @endif
@endif
@endsection