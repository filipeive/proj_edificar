    ...
    <div class="space-y-8">
        ...
        <div
            ...
            <div class="flex items-center gap-3">
                @if($cell)
                    <a href="{{ route('reports.export.pdf', ['type' => 'cell', 'id' => $cell->id, 'start_date' => $startDate->format('Y-m-d'), 'end_date' => $endDate->format('Y-m-d')]) }}"
                    ...
                    <a href="{{ route('reports.export.excel', ['type' => 'cell', 'id' => $cell->id, 'start_date' => $startDate->format('Y-m-d'), 'end_date' => $endDate->format('Y-m-d')]) }}"
        ...
        @if($cell)
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div
                    ...
                    <div class="relative z-10">
                        <p class="text-[10px] font-black text-green-100 uppercase tracking-widest mb-1">Total Arrecadado
                            (Verificado)</p>
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 relative group overflow-hidden">
                    ...
                    <div class="relative z-10">
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Doados Distintos</p>
                        <h3 class="text-4xl font-black text-gray-900 tracking-tighter">
                            {{ $contributions->unique('user_id')->count() }}<span
                                class="text-lg ml-1 uppercase opacity-50 font-bold">Membros</span>
                        </h3>
                        <p class="mt-4 text-[10px] font-black text-blue-600 uppercase tracking-tighter">Alcance Ministerial</p>
                    </div>
                </div>

                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 relative group overflow-hidden">
                    <i
                        class="bi bi-check2-circle absolute right-8 top-1/2 -translate-y-1/2 text-7xl text-gray-50 opacity-50 group-hover:scale-110 transition-transform duration-500"></i>
                    <div class="relative z-10">
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Status de Verificação</p>
                        <h3 class="text-4xl font-black text-gray-900 tracking-tighter">
                            {{ round(($contributions->where('status', 'verificada')->count() / max($contributions->count(), 1)) * 100) }}<span
                                class="text-lg ml-1 uppercase opacity-50">%</span>
                        </h3>
                        <p class="mt-4 text-[10px] font-black text-orange-500 uppercase tracking-tighter">
                            {{ $contributions->where('status', 'pendente')->count() }} Pendentes
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-8 border-b border-gray-50 bg-gray-50/30 flex items-center justify-between">
                    <h3 class="text-sm font-black text-gray-900 uppercase tracking-widest">Extrato de Movimentações</h3>
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors group">
                                    <td class="px-10 py-6">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center font-black text-sm group-hover:bg-blue-600 group-hover:text-white transition-all shadow-sm">
        ...
        @else
            @if(auth()->user()->role === 'admin')
                <div class="bg-white rounded-[2.5rem] p-20 text-center border border-gray-100 shadow-sm">
                    <div
                        class="w-24 h-24 bg-blue-50 text-blue-500 rounded-full flex items-center justify-center mx-auto mb-6 text-4xl">
                        <i class="bi bi-search"></i>
                    </div>
                    <h3 class="text-2xl font-black text-gray-900 mb-2">Aguardando Seleção</h3>
                    <p class="text-gray-500 font-medium max-w-md mx-auto">Escolha uma célula no menu de filtros acima para gerar o
                        relatório consolidado de contribuições.</p>
                </div>
            @endif
        @endif
    </div>
