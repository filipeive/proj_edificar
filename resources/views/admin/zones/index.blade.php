            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    <div class="group bg-white rounded-[2rem] p-7 shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-blue-100 relative overflow-hidden">
                        ...
                        <div class="relative z-10 flex flex-col h-full">
                            ...
                            <div class="space-y-1 mb-6">
                                <span class="text-[10px] font-black text-blue-600 uppercase tracking-widest">{{ $zone->pastor->name ?? 'Pr. Geral' }}</span>
                            ...
                            <div class="grid grid-cols-3 gap-3 mb-6">
                                <div class="bg-gray-50/50 p-3 rounded-xl text-center">
                                    <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-1">Sup.</span>
                                    <span class="text-sm font-black text-gray-900">{{ $zone->supervisions->count() }}</span>
                                </div>
                                <div class="bg-gray-50/50 p-3 rounded-xl text-center">
                                    <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-1">Células</span>
                                    <span class="text-sm font-black text-gray-900">{{ $zone->getTotalCells() }}</span>
                                </div>
                                <div class="bg-gray-50/50 p-3 rounded-xl text-center">
                                    <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-1">Memb.</span>
                ...
                @empty
                    <div class="col-span-full py-20 text-center bg-white rounded-[2.5rem] border border-gray-100 border-dashed">
                        ...
                        <p class="text-gray-500 font-medium mb-8">Comece definindo a primeira zona territorial para seu ministério.</p>
                        <a href="{{ route('zones.create') }}" class="inline-flex items-center bg-blue-600 text-white px-8 py-4 rounded-2xl hover:bg-blue-700 transition-all font-black text-xs uppercase tracking-widest shadow-lg shadow-blue-200">
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-gray-50/50">
                            <th class="px-10 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Descrição da Zona</th>
                            <th class="px-10 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Pastor Coordenador</th>
                            <th class="px-10 py-6 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest">Supervisões</th>
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors group text-sm">
                                ...
                                <td class="px-10 py-6">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-xs font-black text-gray-400 uppercase shadow-inner">
                                            {{ substr($zone->pastor->name ?? 'G', 0, 1) }}
                                        </div>
                                        <span class="font-bold text-gray-700">{{ $zone->pastor->name ?? 'Geral' }}</span>
                                    </div>
                                </td>
                                <td class="px-10 py-6 text-center font-black text-gray-900">{{ $zone->supervisions->count() }}</td>
                                <td class="px-10 py-6 text-center font-black text-blue-600">{{ $zone->getTotalCells() }}</td>
                                <td class="px-10 py-6 text-right">
                                    <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                        <a href="{{ route('zones.show', $zone) }}" class="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="{{ route('zones.edit', $zone) }}" class="w-10 h-10 rounded-xl bg-gray-50 text-gray-400 flex items-center justify-center hover:bg-yellow-500 hover:text-white transition-all">
