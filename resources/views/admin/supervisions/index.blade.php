            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    <div
                        ...
                        <div class="relative z-10 flex flex-col h-full">
                            <div class="flex items-start justify-between mb-6">
                                <div
                                    class="w-14 h-14 rounded-2xl bg-blue-100 text-blue-600 flex items-center justify-center text-2xl font-black shadow-sm group-hover:rotate-6 transition-transform">
                                ...
                                <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <form action="{{ route('supervisions.destroy', $supervision) }}" method="POST"
                                        id="delete-form-{{ $supervision->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            onclick="confirmDelete('{{ route('supervisions.destroy', $supervision) }}', 'delete-form-{{ $supervision->id }}')"
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-6">
                                ...
                                <div class="bg-gray-50/50 p-4 rounded-2xl">
                                    <span
                                        class="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">Membros</span>
                                    <span
                                        class="text-xl font-black text-gray-900">{{ $supervision->cells->sum(fn($c) => $c->members->count()) }}</span>
                                </div>
                            </div>
                ...
                @empty
                    <div class="col-span-full py-20 text-center bg-white rounded-[2.5rem] border border-gray-100 border-dashed">
                        ...
                        <p class="text-gray-500 font-medium mb-8">Comece criando a primeira supervisão para organizar a
                            estrutura.</p>
                        <a href="{{ route('supervisions.create') }}"
                    ...
                    <thead>
                        <tr class="bg-gray-50/50">
                            <th class="px-10 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                                Identificação</th>
                            <th class="px-10 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Zona
                                Afiliada</th>
                            <th
