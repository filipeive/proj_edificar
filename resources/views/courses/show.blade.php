                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="px-8 py-4 bg-gray-50/50 border-b border-gray-100">
                        <form action="{{ route('courses.show', $course) }}" method="GET" class="relative">
                            ...
                            @if($search)
                                <a href="{{ route('courses.show', $course) }}" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                                            ...
                                            @if(auth()->user()->role === 'admin' || auth()->user()->role === 'pastor')
                                                <td class="px-8 py-6">
                                                    <input type="checkbox" name="enrollment_ids[]" value="{{ $enrollment->id }}" form="bulkActionForm"
                                                        class="enrollment-checkbox rounded-lg border-gray-300 text-orange-600 shadow-sm focus:border-orange-300 focus:ring focus:ring-orange-200 focus:ring-opacity-50 transition-all cursor-pointer w-5 h-5">
                                                </td>
                                            @endif
                                            <td class="px-8 py-6">
                                                                ...
                                                                <div class="py-1 text-left">
                                                                    @foreach(['enrolled' => 'Ativo', 'completed' => 'Concluído', 'dropped' => 'Desistir'] as $s => $label)
                                                                        <form action="{{ route('enrollments.status', $enrollment) }}" method="POST">
                                                                            ...
                                                                            <button type="submit" 
                                                                                class="w-full text-left px-4 py-3 text-xs font-bold uppercase tracking-widest hover:bg-gray-50 transition-colors {{ $enrollment->status == $s ? 'text-orange-600 bg-orange-50/50' : 'text-gray-600' }}">
                                                                                {{ $label }}
                                                                            </button>
                                                                        </form>
                                                                    @endforeach
